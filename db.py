...
cursor.executemany('INSERT INTO students (student_id, username, password, name, email) VALUES (?, ?, ?, ?, ?)', students)

# Rooms
for i in range(1, 13):  # 12 rooms
    room_number = f'R{i:03d}'
    capacity = 1 if i < 10 else (2 if i <= 25 else 3)
    cursor.execute('INSERT INTO rooms (room_number, status, capacity, current_occupancy) VALUES (?, ?, ?, ?)',
                   (room_number, 'available', capacity, 0))
...
for i in range(6):  # 6 bookings
    ...
    cursor.execute('INSERT INTO bookings (student_id, room_id, status, booking_date) VALUES (?, ?, ?, ?)',
...
cursor.executemany('INSERT INTO complaints (student_id, complaint_text, status, created_at) VALUES (?, ?, ?, ?)', complaints)
