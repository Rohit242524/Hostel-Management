...
def setup_routes(app):
                ...
                try:
                    ...
                    if "UNIQUE constraint failed: students.student_id" in str(e):
                        return render_template(
                            ...
                            error="Student ID (Roll Number) already exists",
                        )
                    elif "UNIQUE constraint failed: students.username" in str(e):
                        return render_template(
                            "register_student.html", error="Username already exists"
                        )
                    else:
                        return render_template(
                            "register_student.html", error="Registration failed"
                        )
    ...
    def admin_dashboard():
        ...
        with get_db() as conn:
            students = conn.execute(
                'SELECT id, COALESCE(student_id, "Not Set") AS student_id, username, name, email FROM students'
            ).fetchall()
    ...
    def book_room():
        if "user" not in session or session["user_type"] != "student":
            return jsonify(
                {"success": False, "message": "Please log in to book a room."}
            )
            ...
            with get_db() as conn:
                ...
                if request.method == "POST":
                    ...
                    return jsonify(
                        {
                            "success": True,
                            "message": "Room booking requested! Waiting for admin approval.",
                        }
                    )
        ...
        try:
            with get_db() as conn:
                if strategy == "fcfs":
                    ...
                    print(
                        "FCFS Bookings Order:",
                        [(b["id"], b["booking_date"]) for b in bookings],
                    )
                elif strategy == "sjf":
                    bookings = conn.execute(
                        """
    ...
    def load_section():
        try:
            if "user" not in session or session["user_type"] != "admin":
                return (
                    jsonify({"error": "Unauthorized", "redirect": url_for("login")}),
                    403,
                )
            ...
            with get_db() as conn:
                if section == "students":
                    ...
                    return render_template(
                        "handle_complaints.html", complaints=complaints
                    )
                else:
